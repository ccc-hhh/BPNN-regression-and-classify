...
def train(model, epochs, x_train, y_train):
    ...
    torch.save(state, "D:/pycharm/数据/BPNN.pt")
    # plt.pause(0)
...
def test(model, x_test, y_test, path):
    ...
    plt.plot(x, y_test.detach().numpy(), color='blue', marker='.', label="test data")
    plt.plot(x, output.detach().numpy(), color='red', marker='.', label="predict data")
    plt.xticks([])
    ...
    plt.legend(loc="upper right")
    plt.savefig('D:/pycharm/数据/Adam_test.png')
    plt.show()
...
def main():
    ...
    if os.path.exists(path_adam):
        ...
        train(net, epochs=1000, x_train=x_train_normalized, y_train=y_train_normalized)
        test(net, x_test_normalized, y_test_normalized, path_adam)
